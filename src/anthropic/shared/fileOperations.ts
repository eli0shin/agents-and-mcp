...
export function getUserConfigPath(...segments: string[]): string {
  return join(homedir(), '.config', 'anthropic-provider', ...segments);
}
...
export function getUserDataPath(...segments: string[]): string {
  ...
  let baseDir: string;
  if (platform === 'darwin') {
    baseDir = join(homedir(), 'Library', 'Application Support', 'anthropic-provider');
  } else if (platform === 'win32') {
    baseDir = join(homedir(), 'AppData', 'Roaming', 'anthropic-provider');
  } else {
    // Linux and other Unix-like systems
    baseDir = join(homedir(), '.local', 'share', 'anthropic-provider');
  }
